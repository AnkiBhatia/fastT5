...
class PegasusDecoderInit(torch.nn.Module):
...
class PegasusDecoder(torch.nn.Module):
    def __init__(self, decoder_sess):
        super().__init__()
        self.decoder = decoder_sess
...
# config = PegasusConfig.from_pretrained(model_or_model_path)


class OnnxPegasus(PegasusForConditionalGeneration):
    ...
    def __init__(self, model_or_model_path, onnx_model_sessions):
        ...
        self.encoder = PegasusEncoder(encoder_sess)
        self.decoder = PegasusDecoder(decoder_sess)
        self.decoder_init = PegasusDecoderInit(decoder_sess_init)
...
def get_onnx_model(model_name_or_path, quantized=True):
    ...
    model = OnnxPegasus(model_name_or_path, model_sessions)
