...
def create_pegasus_encoder_decoder(pretrained_version=model_name):
    ...
    model = PegasusForConditionalGeneration.from_pretrained(pretrained_version)
...
def generate_onnx_representation(pretrained_version=None, model=None):
    ...
    sample_input = """PG&E stated it scheduled the blackouts in response to forecasts for high winds amid dry conditions. The aim is to reduce the risk of wildfires. Nearly 800 thousand customers were scheduled to be affected by the shutoffs which were expected to last through at least midday tomorrow."""
    ...
    past_key_values = (Pegasus_block,) * model_config.num_decoder_layers
